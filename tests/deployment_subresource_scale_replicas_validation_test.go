...
func TestSubresourceReplicaCountValidation(t *testing.T) {
	featureTrueValidation := features.New("Check Scale Validation").
		Setup(func(ctx context.Context, t *testing.T, config *envconf.Config) context.Context {
			deployment, err := util.GetDeployment("./scenarios/check-replicas-scale-subresource/deployment.yaml")
			if err != nil {
				t.Fatal(err)
			}
		...
		Assess("Check Number of Replicas", func(ctx context.Context, t *testing.T, config *envconf.Config) context.Context {
			validationPrecondition, err := util.GetKubernetesResourcePrecondition("./scenarios/check-replicas-scale-subresource/validation.yaml")
			if err != nil {
				t.Fatal(err)
			}
	...
	featureFalseValidation := features.New("Check Scale Validation With 2 Replicas").
		Setup(func(ctx context.Context, t *testing.T, config *envconf.Config) context.Context {
			deployment, err := util.GetDeployment("./scenarios/check-replicas-scale-subresource/deployment.yaml")
			if err != nil {
				t.Fatal(err)
		...
		Assess("Check Number of Replicas", func(ctx context.Context, t *testing.T, config *envconf.Config) context.Context {
			...
			result, selectionPreconditionValidationFailures, validationFailure, err := celvalidator.GetKubernetesResourcePreconditionResult(validationPrecondition, client.RESTConfig())
			if err != nil {
				t.Fatal(err)
			}
			if result != false {
